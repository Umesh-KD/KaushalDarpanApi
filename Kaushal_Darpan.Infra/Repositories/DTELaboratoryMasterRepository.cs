...
namespace Kaushal_Darpan.Infra.Repositories
{
    public class DTELaboratoryMasterRepository : IDTELaboratoryMasterRepository
    {
        ...
        public async Task<DataTable> GetLabDetailsByUserID(LabDetailsSearchModel model)
        {
            _actionName = "GetLabDetailsByUserID(LabDetailsSearchModel model)";
            return await Task.Run(async () =>
            {
                try
                {
                    ...
                    using (var command = await _dbContext.CreateCommandAsync())
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.CommandText = "USP_DTELabMaster_Operation";
                        command.Parameters.AddWithValue("@ActionName", "GetLabDetailsByUserID");
