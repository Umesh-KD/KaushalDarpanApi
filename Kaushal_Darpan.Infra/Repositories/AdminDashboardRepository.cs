...
namespace Kaushal_Darpan.Infra.Repositories
{
    public class AdminDashboardRepository : IAdminDashboardRepository
    {
        ...
        public async Task<DataTable> GetBTEREstablishmentDashboard(EstablishmentDashboardBTERDataModel model)
        {
            _actionName = "GetBTEREstablishmentDashboard(EstablishmentDashboardBTERDataModel model)";
            try
            {
                DataTable dataTable = new DataTable();
                using (var command = await _dbContext.CreateCommandAsync())
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.CommandText = "USP_BTER_EstablishmentDashboard";
                    command.Parameters.AddWithValue("@DepartmentID", model.DepartmentID);
                    command.Parameters.AddWithValue("@EndTermID", model.EndTermID);
                    command.Parameters.AddWithValue("@Eng_NonEng", model.Eng_NonEng);
                    command.Parameters.AddWithValue("@RoleID", model.RoleID);
