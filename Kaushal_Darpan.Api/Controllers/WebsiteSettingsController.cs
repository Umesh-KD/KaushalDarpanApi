...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class WebsiteSettingsController : BaseController
    {
        ...
        public async Task<ApiResult<DataTable>> GetAllSearchData(WebsiteSettingDataModel request)
        {
           ActionName = "GetAllSearchData(DateConfigurationModel request)";
            var result = new ApiResult<DataTable>();
            try
            {
        ...
        [HttpPost("GetAllDataOrders")]
        public async Task<ApiResult<DataTable>> GetAllDataOrders(WebsiteSettingDataModel request)
        {
            ActionName = "GetAllData(DateConfigurationModel request)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.WebsiteSettingsRepository.GetAllDataOrders(request);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
