...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class SetExamAttendanceController : BaseController
    {
        ...
        [HttpPost("GetTimeTableBranchesData")]
        public async Task<ApiResult<DataTable>> GetExamSGetTimeTableBranchesDatatudentData([FromBody] SetExamAttendanceSearchModel body)
        {
            ActionName = "GetExamSGetTimeTableBranchesDatatudentData([FromBody] SetExamAttendanceSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.SetExamAttendanceRepository.GetTimeTableBranchesData(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetBlankReportData_Admin")]
        public async Task<ApiResult<DataTable>> GetBlankReportData_Admin([FromBody] SetExamAttendanceSearchModel body)
        {
            ActionName = "GetBlankReportData_Admin([FromBody] SetExamAttendanceSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.SetExamAttendanceRepository.GetBlankReportData_Admin(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
