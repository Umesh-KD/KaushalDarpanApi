...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class RenumerationAccountsRevalController : BaseController
    {
        public override string PageName => "RenumerationAccountsRevalController";
        public override string ActionName { get; set; }

        private readonly IMapper _mapper;
        private readonly IUnitOfWork _unitOfWork;

        public RenumerationAccountsRevalController(IMapper mapper, IUnitOfWork unitOfWork)
        {
            _mapper = mapper;
        ...
        public async Task<ApiResult<List<RenumerationAccountsModel>>> GetAllData([FromBody] RenumerationAccountsRequestModel filterModel)
        {
            ...
            try
            {
                ...
                var data = await _unitOfWork.RenumerationAccounts.GetAllData_Reval(filterModel);
        ...
        public async Task<ApiResult<bool>> SaveDataApprovedFromAccounts(RenumerationAccountsSaveModel request)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                {
                    //check dblicate 
                    var hasDublicate = await _unitOfWork.RenumerationAccounts.HasDblicateTvNoAndVoucharNo_Reval(request);
                    if (hasDublicate != 0)
                    {
                        result.State = EnumStatus.Error;
        ...
        public async Task<ApiResult<bool>> UpdateDataApprovedFromAccounts(RenumerationAccountsSaveModel request)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    var isSave = await _unitOfWork.RenumerationAccounts.UpdateDataApprovedFromAccounts_Reval(request);
                    await _unitOfWork.SaveChangesAsync();  // Commit changes if everything is successful
