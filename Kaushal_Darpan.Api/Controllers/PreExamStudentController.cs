...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class PreExamStudentController : BaseController
    {
        ...
        [HttpPost("GetPreExamStudentReport")]
        public async Task<ApiResult<DataTable>> GetPreExamStudentReport(PreExamStudentModel model)
        {
            ActionName = "GetPreExamStudent()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.PreExamStudentRepository.GetPreExamStudentReport(model);
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpGet("HaveOptionalSubject/{StudentExamID}")]
        public async Task<ApiResult<bool>> HaveOptionalSubject(int StudentExamID)
        {
            return await Task.Run(async () =>
            {
                var result = new ApiResult<bool>();
                try
                {
                    var data = await _unitOfWork.PreExamStudentRepository.HaveOptionalSubject(StudentExamID);
                    if (data != null)
                    {
                        result.Data = data;
                        result.State = EnumStatus.Success;
        ...
        {
            ActionName = "GetPartiallyDetainedStudentList(GetPartiallyDetainedStudentDataModel model)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.PreExamStudentRepository.GetPartiallyDetainedStudentList(model);
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
