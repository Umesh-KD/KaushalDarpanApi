...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITICenterAllocationcontroller : BaseController
    {
        ...
        [HttpPost("GetCenterCollegeMapList")]
        public async Task<ApiResult<DataTable>> GetCenterCollegeMapList([FromBody] ITICenterAllocationSearchFilter filterModel)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.ITICenterAllocationRepository.GetCenterCollegeMapList(filterModel);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
