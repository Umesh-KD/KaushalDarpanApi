...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class CounsellingApplicationFormController : BaseController
    {
        ...
        [HttpPost("SavePersonalDetailsFromAdmin")]
        public async Task<ApiResult<int>> SavePersonalDetailsFromAdmin([FromBody] CounsellingApplicationFormDataModel request)
        {
            ActionName = "SaveData([FromBody] HRMaster request)";
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    result.Data = await _unitOfWork.CounsellingApplicationFormRepository.SavePersonalDetailsFromAdmin(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data > 0)
                    {
                        result.State = EnumStatus.Success;
        ...
        [HttpPost("UnlockApplication_Counselling")]
        public async Task<ApiResult<bool>> UnlockApplication_Counselling(CounsellingApplicationSearchModel model)
        {
            ActionName = "UnlockApplication_Counselling(CounsellingApplicationSearchModel model)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<bool>();
                try
                {
                    result.Data = await _unitOfWork.CounsellingApplicationFormRepository.UnlockApplication_Counselling(model);
                    await _unitOfWork.SaveChangesAsync();
