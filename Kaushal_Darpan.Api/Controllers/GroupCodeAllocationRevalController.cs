...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class GroupCodeAllocationRevalController : BaseController
    {
        public override string PageName => "GroupCodeAllocationReavlController";
        public override string ActionName { get; set; }

        private readonly IMapper _mapper;
        private readonly IUnitOfWork _unitOfWork;

        public GroupCodeAllocationRevalController(IMapper mapper, IUnitOfWork unitOfWork)
        {
            _mapper = mapper;
        ...
        [HttpPost("GetAllData")]
        public async Task<ApiResult<List<GroupCodeAllocationAddEditModel_Reval>>> GetAllData([FromBody] GroupCodeAllocationSearchModel filterModel)
        {
            ActionName = "GetAllData([FromBody] GroupCodeAllocationSearchModel filterModel)";
            var result = new ApiResult<List<GroupCodeAllocationAddEditModel_Reval>>();
            try
            {
                // Pass the entire model to the repository
                var data = await _unitOfWork.GroupCodeAllocationRevalRepository.GetAllData(filterModel);
        ...
        public async Task<ApiResult<List<GroupCodeAddEditModel>>> GetPartitionData([FromBody] GroupCodeAllocationSearchModel filterModel)
        {
            ...
            try
            {
                ...
                // Pass the entire model to the repository
                var data = await _unitOfWork.GroupCodeAllocationRevalRepository.GetPartitionData(filterModel);
        ...
        private List<GroupCodeAddEditModel> SetThenOptimizePartitionData(List<GroupCodeAddEditModel> studentPapers, int PartitionSize)
        {
            ...
            foreach (var distinctSubjectCode in distinctSubjectCodes)
            {
                ...
                foreach (var subjectStudentPaper in subjectStudentPapers)
                {
                    ...
                    //                                        .Contains(x) == false)
                    //                            .ToList();
                    var currentStudentExamPaperRevaluationIDs = subjectStudentPaper.StudentExamPaperRevaluationIDs.Split(',')
                                                .Where(x => mergedGroupNo_StudentExamPaperRevaluationIDs
                                                            .Contains(x) == false)
                                                .ToList();
                    mergedRows.AddRange(currentStudentExamPaperRevaluationIDs);//add current

                    // get all subject code for selected but not already merged
                    var subjectNotAlreadyMergedStudentPapers = subjectStudentPapers
                                                .Where(x => x.StudentExamPaperRevaluationIDs.Split(',')
                                                                .All(id => mergedGroupNo_StudentExamPaperRevaluationIDs.Contains(id) == false)
                                                           && x.StudentExamPaperRevaluationIDs.Split(',')
                    ...
                    foreach (var subjectNotAlreadyMergedStudentPaper in subjectNotAlreadyMergedStudentPapers)
                    {
                        ...
                        {
                            mergedRows.AddRange(subjectNotAlreadyMergedStudentPaper.StudentExamPaperRevaluationIDs.Split(','));
                        }
                    }
                    ...
                    if (mergedRows.Count > 0)
                    {
                        ...
                        if (subjectStudentPapersMergeds.Count > 0)
                        {
                            foreach (var subjectStudentPapersMerged in subjectStudentPapersMergeds)
                            {
                                ...
                                singleSP.StudentExamPaperRevaluationIDs += subjectStudentPapersMerged.StudentExamPaperRevaluationIDs + ",";//merge
                            ...
                            singleSP.StudentExamPaperRevaluationIDs = singleSP.StudentExamPaperRevaluationIDs.Trim().TrimEnd(',');
                            //add in new list
