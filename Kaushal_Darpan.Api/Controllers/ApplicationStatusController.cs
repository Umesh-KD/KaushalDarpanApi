...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ApplicationStatusController : BaseController
    {
        ...
        [HttpPost("EditQualificationList")]
        public async Task<ApiResult<DataTable>> EditQualificationList([FromBody] StudentSearchModel body)
        {
            ActionName = "GetAllData([FromBody] StudentSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                // Pass the entire model to the repository
                result.Data = await _unitOfWork.ApplicationStatusRepository.EditQualificationList(body);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        [HttpPost("AddQualificationDetails")]
        public async Task<ApiResult<DataTable>> AddQualificationDetails([FromBody] StudentSearchModel body)
        {
            ActionName = "GetAllData([FromBody] StudentSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                // Pass the entire model to the repository
                result.Data = await _unitOfWork.ApplicationStatusRepository.AddQualificationDetails(body);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
