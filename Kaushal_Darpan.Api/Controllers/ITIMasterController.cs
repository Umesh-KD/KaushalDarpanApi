...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITIMasterController : BaseController
    {
        ...
        [HttpPost("getTradeList")]
        public async Task<ApiResult<DataTable>> getTradeList([FromBody] ITISearchModel body)
        {
            ActionName = "getTradeList()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITIMasterRepository.getTradeList(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetITIPaperUpload_Reports")]
        public async Task<ApiResult<DataTable>> GetITIPaperUpload_Reports([FromBody] ITIPaperUploadSearchModel body)
        {
            ActionName = "GetCenterWisePaperDetail([FromBody] CenterWisePaperDetailModal)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITIMasterRepository.GetITIPaperUpload_Reports(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        public async Task<ApiResult<DataTable>> GetDashboardData([FromBody] ITISearchModel body)
        {
            ActionName = "GetDashboardData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITIMasterRepository.GetDashboardData(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
