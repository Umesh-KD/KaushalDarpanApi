...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class UserRequestController : BaseController
    {
        ...
        [HttpPost("UserRequestJoiningApprove_ITI_EM")]
        public async Task<ApiResult<int>> UserRequestJoiningApprove_ITI_EM([FromBody] RequestUpdateStatus request)
        {
            ActionName = "StafffJoiningRequestUpdateAndPromotions([FromBody] RequestUpdateStatus request)";
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    result.Data = await _unitOfWork.UsersRequest.UserRequestJoiningApprove_ITI_EM(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data > 0)
                    {
                        result.State = EnumStatus.Success;
