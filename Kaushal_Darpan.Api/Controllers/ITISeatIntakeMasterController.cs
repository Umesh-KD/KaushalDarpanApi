...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITISeatIntakeMasterController : BaseController
    {
        ...
        [HttpPost("GetAllDataAdmissionList")]
        public async Task<ApiResult<DataTable>> GetAllDataAdmissionList(BTERSeatIntakeSearchModel request)
        {
            ActionName = "GetAllData(SeatIntakeSearchModel request)";
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    var data = await _unitOfWork.ITISeatIntakeMasterRepository.GetAllDataAdmissionList(request);
                    if (data != null)
                    {
        ...
        [HttpPost("GetTradeCollegesMaster")]
        public async Task<ApiResult<DataTable>> GetTradeCollegesMaster([FromBody] ITICollegeTradeSearchModel body)
        {
            ActionName = "GetTradeAndColleges()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITISeatIntakeMasterRepository.GetTradeCollegesMaster(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("SaveSeatIntakeDataMaster")]
        public async Task<ApiResult<int>> SaveSeatIntakeDataMaster([FromBody] BTERSeatIntakeDataModel request)
        {
            ActionName = "SaveSeatIntakeData([FromBody] SeatIntakeDataModel request)";
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    result.Data = await _unitOfWork.ITISeatIntakeMasterRepository.SaveSeatIntakeDataMaster(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data > 0)
                    {
                        result.State = EnumStatus.Success;
        ...
        [HttpPost("GetActiveSeatIntakeAdmission")]
        public async Task<ApiResult<DataTable>> GetActiveSeatIntakeAdmission([FromBody] BTERSeatIntakeSearchModel body)
        {
            ActionName = "GetActiveSeatIntake()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITISeatIntakeMasterRepository.GetActiveSeatIntakeAdmission(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
