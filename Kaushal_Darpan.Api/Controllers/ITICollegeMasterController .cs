...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITICollegeMasterController : BaseController
    {
        ...
        //[HttpPost("ItiSearchCollege")]
        [HttpPost("ItiVacantSeatForDirectAdmission")]
        public async Task<ApiResult<DataTable>> ItiVacantSeatForDirectAdmission([FromBody] ItiCollegeModel model)
        {
            ActionName = "ItiVacantSeatForDirectAdmission()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.ITICollegeMasterRepository.ItiVacantSeatForDirectAdmission(model);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        [HttpPost("GetAllEstablishmentIti")]
        public async Task<ApiResult<DataTable>> GetAllEstablishmentIti([FromBody] ItiEstablishmentSearchModel model)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.ITICollegeMasterRepository.GetAllEstablishmentIti(model);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        public async Task<ApiResult<DataTable>> ITIPlanningBankGuaranteeList([FromBody] ITIPlanningBankGuaranteeSearchList body)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
        ...
        [HttpPost("ITIPlanningBankGuaranteeReturn")]
        public async Task<ApiResult<DataTable>> ITIPlanningBankGuaranteeReturn([FromBody] ITIPlanningBankGuaranteeReturn body)
        {
            ActionName = "ITIPlanningBankGuaranteeReport()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITICollegeMasterRepository.ITIPlanningBankGuaranteeReturn(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
    ...
    GetBankGuaranteeConsolidatedReport(
        ...
        [HttpPost("ChangeCampusByID")]
        public async Task<ApiResult<bool>> ChangeCampusByID([FromBody] ITICampusStatusModel model)
        {
            ActionName = "UpdateCampusStatusByID";
            ...
            try
            {
                result.Data = await _unitOfWork.ITICollegeMasterRepository.ChangeCampusByID(model);
                await _unitOfWork.SaveChangesAsync();
