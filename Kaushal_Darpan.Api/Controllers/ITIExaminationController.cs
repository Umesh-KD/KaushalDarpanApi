...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITIExaminationController : BaseController
    {
        ...
        [HttpPost("GetDetails")]
        public async Task<ApiResult<DataTable>> GetDetails([FromBody] ITIRevaluationDataModel body)
        {
            ActionName = "GetTeacherForExaminer()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.ITIExaminationRepository.GetDetails(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Error;
