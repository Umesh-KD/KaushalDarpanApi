...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class GuestRoomManagementController : BaseController
    {
        ...
        [HttpPost("ReqApproveByAdmin")]
        public async Task<ApiResult<int>> ReqApproveByAdmin([FromBody] GuestApplyForGuestRoomDataModel request)
        {
            ActionName = "ReqApproveByAdmin([FromBody] GuestApplyForGuestRoomDataModel request)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<int>();
                try
                {
                    ...
                    result.Data = await _unitOfWork.GuestRoomManagementRepository.ReqApproveByAdmin(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data > 0)
                    {
                        result.State = EnumStatus.Success;
        ...
        [HttpPost("GetGuestHouseRoomAvailabilityData")]
        public async Task<ApiResult<DataTable>> GetGuestHouseRoomAvailabilityData([FromBody] GuestRoomSeatSearchModel body)
        {
            ActionName = "GetGuestHouseRoomAvailabilityData([FromBody] GuestRoomSeatSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.GuestRoomManagementRepository.GetGuestHouseRoomAvailabilityData(body));
        ...
        [HttpPost("ReservedRoomCheckIn")]
        public async Task<ApiResult<RoomReservationSaveResponse>> ReservedRoomCheckIn([FromBody] GuestApplyForGuestRoomDataModel request)
        {
            ActionName = "ReservedRoomCheckIn([FromBody] GuestApplyForGuestRoomDataModel request)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<RoomReservationSaveResponse>();
                try
                {
                    ...
                    result.Data = await _unitOfWork.GuestRoomManagementRepository.ReservedRoomCheckIn(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data.State == 1)
                    {
                        result.State = EnumStatus.Success;
        ...
        [HttpPost("UnreserveGuestHouseRoom")]
        public async Task<ApiResult<bool>> UnreserveGuestHouseRoom([FromBody] UnreserveGuestRoomDataModel request)
        {
            ActionName = " UnreserveGuestHouseRoom([FromBody] UnreserveGuestRoomDataModel request)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<bool>();
                try
                {
                    result.Data = await _unitOfWork.GuestRoomManagementRepository.UnreserveGuestHouseRoom(request);
                    await _unitOfWork.SaveChangesAsync();

                    if (result.Data)
                    {
                        result.State = EnumStatus.Success;
                        result.Message = "Reservation Cancellation Success .!";
                    }
                    else
                    {
                        result.State = EnumStatus.Error;
