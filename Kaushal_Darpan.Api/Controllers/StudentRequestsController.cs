...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class StudentRequestsController : BaseController
    {
        ...
        [HttpPost("GetRoomPreference")]
        public async Task<ApiResult<DataTable>> GetRoomPreference([FromBody] GetMeritDataModel_Hostel body)
        {
            ActionName = "GetRoomPreference([FromBody] GetMeritDataModel_Hostel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.iStudentRequestsRepository.GetRoomPreference(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("WithdrawHostelRequest")]
        public async Task<ApiResult<bool>> WithdrawHostelRequest([FromBody] DeallocateRoomDataModel request)
        {
            ActionName = "WithdrawHostelRequest([FromBody] DeallocateRoomDataModel request)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<bool>();
                try
                {
                    result.Data = await _unitOfWork.iStudentRequestsRepository.WithdrawHostelRequest(request);
                    await _unitOfWork.SaveChangesAsync();
                    if (result.Data)
                    {
                        result.State = EnumStatus.Success;
        ...
        [HttpPost("HostelStudentMeritListWardenView")]
        public async Task<ApiResult<DataTable>> HostelStudentMeritListWardenView([FromBody] SearchStudentApplyForHostel body)
        {
            ActionName = "HostelStudentMeritListWardenView([FromBody] SearchStudentApplyForHostel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.iStudentRequestsRepository.HostelStudentMeritListWardenView(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        public async Task<ApiResult<DataTable>> GetStudentdata([FromBody] GetMeritDataModel_Hostel body)
        {
            ActionName = "GetStudentdata([FromBody] GetMeritDataModel_Hostel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.iStudentRequestsRepository.GetStudentdata(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetStudentHostelFeeData")]
        public async Task<ApiResult<DataTable>> GetStudentHostelFeeData([FromBody] GetMeritDataModel_Hostel body)
        {
            ActionName = "GetStudentHostelFeeData([FromBody] GetMeritDataModel_Hostel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.iStudentRequestsRepository.GetStudentHostelFeeData(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
