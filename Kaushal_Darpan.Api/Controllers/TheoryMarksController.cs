...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class TheoryMarksController : BaseController
    {
        ...
        [HttpPost("GetTheoryAbsentReportData")]
        public async Task<ApiResult<DataTable>> GetTheoryAbsentReportData([FromBody] TheoryAbsentReportDataModel body)
        {
            ActionName = "GetTheoryAbsentReportData([FromBody] TheoryAbsentReportDataModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.TheoryMarksRepository.GetTheoryAbsentReportData(body));
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Warning;
                    result.Message = "No record found.!";
