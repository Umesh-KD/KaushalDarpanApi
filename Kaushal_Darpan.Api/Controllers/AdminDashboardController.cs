...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class AdminDashboardController : BaseController
    {
        ...
        [HttpPost("GetBTEREstablishmentDashboard")]
        public async Task<ApiResult<DataTable>> GetBTEREstablishmentDashboard([FromBody] EstablishmentDashboardBTERDataModel model)
        {
            ActionName = "GetBTEREstablishmentDashboard([FromBody] EstablishmentDashboardBTERDataModel model)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.AdminDashboardRepository.GetBTEREstablishmentDashboard(model);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
