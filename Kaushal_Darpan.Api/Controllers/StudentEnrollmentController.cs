...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class StudentEnrollmentController : BaseController
    {
        ...
        public StudentEnrollmentController(IMapper mapper, IUnitOfWork unitOfWork)
        {
            _mapper = mapper;
        ...
        [RoleActionFilter(EnumRole.DTE_Eng, EnumRole.DTE_NonEng, EnumRole.ITCell_Eng, EnumRole.ITCell_NonEng)]
        ...
        public async Task<ApiResult<DataTable>> GetStudentAdmitted(PreExamStudentModel model)
        {
            ...
            try
            {
                result.Data = await _unitOfWork.StudentEnrollmentRepository.GetStudentAdmitted(model);
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        public async Task<ApiResult<bool>> SaveAdmittedFinalStudentData([FromBody] List<StudentMarkedModelForJoined> request)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    var isSave = await _unitOfWork.StudentEnrollmentRepository.SaveAdmittedFinalStudentData(request);
        ...
        public async Task<ApiResult<DataTable>> GetPreExamStudentReport(PreExamStudentModel model)
        {
            ...
            try
            {
                result.Data = await _unitOfWork.StudentEnrollmentRepository.GetPreExamStudentReport(model);
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
