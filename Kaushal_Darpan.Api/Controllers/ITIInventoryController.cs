...
namespace Kaushal_Darpan.Api.Controllers
{
    ...
    public class ITIInventoryController : BaseController
    {
        ...
        public async Task<ApiResult<int>> SaveItemsMaster([FromBody] DTEItemsModel   request)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    {
                        result.State = EnumStatus.Error;
                        result.ErrorMessage = "This item has already been approved and cannot be edited.";
                    }
                    else
                    {
                        result.State = EnumStatus.Error;
        ...
        public async Task<ApiResult<DataTable>> GetAllinventoryIssueHistory([FromBody] inventoryIssueHistorySearchModel body)
        {
            ...
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAllinventoryIssueHistory(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        public async Task<ApiResult<DataTable>> GetAll_INV_GetCommonIssueDDL([FromBody] inventoryIssueHistorySearchModel body)
        {
            ...
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAll_INV_GetCommonIssueDDL(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetAllDDL")]
        public async Task<ApiResult<DataTable>> GetAllDDL(DTEItemsSearchModel SearchReq)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.i_ITIInventoryRepository.GetAllDDL(SearchReq);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        public async Task<ApiResult<DataTable>> GetConsumeItemList(DTEItemsSearchModel SearchReq)
        {
            ...
            try
            {
                result.Data = await _unitOfWork.i_ITIInventoryRepository.GetConsumeItemList(SearchReq);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        public async Task<ApiResult<DataTable>> GetAll_INV_returnItem([FromBody] ItemsIssueReturnModels body)
        {
            ...
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAll_INV_returnItem(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        public async Task<ApiResult<DataTable>> GetInventoryIssueItemList([FromBody] inventoryIssueHistorySearchModel body)
        {
            ...
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetInventoryIssueItemList(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetConsumeItemListNew")]
        public async Task<ApiResult<DataTable>> GetConsumeItemListNew(DTEItemsSearchModel SearchReq)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await _unitOfWork.i_ITIInventoryRepository.GetConsumeItemListNew(SearchReq);
                if (result.Data.Rows.Count > 0)
                {
                    result.State = EnumStatus.Success;
                    result.Message = Constants.MSG_DATA_LOAD_SUCCESS;
        ...
        [HttpPost("GetAll_INV_GetCommonIssueDDLNew")]
        public async Task<ApiResult<DataTable>> GetAll_INV_GetCommonIssueDDLNew([FromBody] inventoryIssueHistorySearchModel body)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAll_INV_GetCommonIssueDDLNew(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetInventoryIssueItemListNew")]
        public async Task<ApiResult<DataTable>> GetInventoryIssueItemListNew([FromBody] inventoryIssueHistorySearchModel body)
        {
            ActionName = "GetAllinventoryIssueHistoryNew([FromBody] inventoryIssueHistorySearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetInventoryIssueItemListNew(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetAll_INV_returnItemNew")]
        public async Task<ApiResult<DataTable>> GetAll_INV_returnItemNew([FromBody] ItemsIssueReturnModels body)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAll_INV_returnItemNew(body));
                await _unitOfWork.SaveChangesAsync();
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
        ...
        [HttpGet("GetIssueItemListPermanent/{EquipmentsId}/{ItemCategoryId}/{TradeId}")]
        public async Task<ApiResult<DataTable>> GetIssueItemListPermanent(int EquipmentsId, int ItemCategoryId, int TradeId)
        {
            ActionName = "GetIssueItemListPermanent(int itemId)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetIssueItemListPermanent(EquipmentsId, ItemCategoryId, TradeId));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetIssueSubmitPermanent")]
        public async Task<ApiResult<DataTable>> GetIssueSubmitPermanent([FromBody] ItemsIssueReturnModels body)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetIssueSubmitPermanent(body));
                await _unitOfWork.SaveChangesAsync();
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
        ...
        [HttpPost("DownloadSR6ReportData_pdf")]
        public async Task<ApiResult<string>> DownloadSR6ReportData_pdf([FromBody] inventoryIssueHistorySearchModel body)
        {
            ActionName = "DownloadSR6ReportData_pdf([FromBody] inventoryIssueHistorySearchModel body)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<string>();
                try
                {
                    var data = await _unitOfWork.i_ITIInventoryRepository.DownloadSR6ReportData_pdf(body);
        ...
        [HttpPost("Download_SR5ReportData_pdf")]
        public async Task<ApiResult<string>> Download_SR5ReportData_pdf([FromBody] inventoryIssueHistorySearchModel body)
        {
            ActionName = "Download_SR5ReportData_pdf([FromBody] inventoryIssueHistorySearchModel body)";
            return await Task.Run(async () =>
            {
                var result = new ApiResult<string>();
                try
                {
                    var data = await _unitOfWork.i_ITIInventoryRepository.Download_SR5ReportData_pdf(body);
        ...
        [HttpPost("GetMinRequiredItem_ITI_INV_Report")]
        public async Task<ApiResult<DataTable>> GetMinRequiredItem_ITI_INV_Report([FromBody] MinRequiredItemSearchModel body)
        {
            ActionName = " GetMinRequiredItem_ITI_INV_Report([FromBody] MinRequiredItemSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetMinRequiredItem_ITI_INV_Report(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetConsumableItemAuctionData")]
        public async Task<ApiResult<DataTable>> GetConsumableItemAuctionData([FromBody] DTEItemsSearchModel body)
        {
            ActionName = "GetConsumableItemAuctionData([FromBody] DTEItemsSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetConsumableItemAuctionData(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetConsumableAuctionedItemsData")]
        public async Task<ApiResult<DataTable>> GetConsumableAuctionedItemsData([FromBody] DTEItemsSearchModel body)
        {
            ActionName = "GetConsumableAuctionedItemsData([FromBody] DTEItemsSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetConsumableAuctionedItemsData(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetItemsForHandover_ITI_INV")]
        public async Task<ApiResult<DataTable>> GetItemsForHandover_ITI_INV([FromBody] HandoverItemSearchModel body)
        {
            ActionName = "GetItemsForHandover_ITI_INV([FromBody] HandoverItemSearchModel body)";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetItemsForHandover_ITI_INV(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
        ...
        {
            ActionName = "HandoverInventoryItems_ITI_INV([FromBody] HandoverInventoryItemsDataModel request)";
            var result = new ApiResult<int>();
            try
            {
                result.Data = await _unitOfWork.i_ITIInventoryRepository.HandoverInventoryItems_ITI_INV(request);
                await _unitOfWork.SaveChangesAsync();
                if (result.Data > 0)
                {
                    result.State = EnumStatus.Success;
        ...
        [HttpPost("GetAllItemsMasterReport")]
        public async Task<ApiResult<DataTable>> GetAllItemsMasterReport([FromBody] DTEItemsSearchModel body)
        {
            ActionName = "GetAllData()";
            var result = new ApiResult<DataTable>();
            try
            {
                result.Data = await Task.Run(() => _unitOfWork.i_ITIInventoryRepository.GetAllItemsMasterReport(body));
                result.State = EnumStatus.Success;
                if (result.Data.Rows.Count == 0)
                {
                    result.State = EnumStatus.Success;
